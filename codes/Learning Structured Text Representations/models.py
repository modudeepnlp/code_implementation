...
# StructureModel class : method(get_feed_dict, build, get_loss)

class StructureModel():
    def __init__(self, config): # config전달 받아서 model 객체를 생성한다.
        self.config = config
        t_variables = {}
    ...
    def build(self):
        ...
        sent_l = self.t_variables['sent_l'] # tf.placeholder(tf.int32, [None, None])
        ...
        [_, _, _, rnn_size] = tokens_input.get_shape().as_list() # shape 리스트로 뽑아내준다.
        ...
        temp1 = tf.zeros([batch_l * max_doc_l, max_sent_l,1], tf.float32) # shape : (?, ?, 1)
        temp2 = tf.zeros([batch_l * max_doc_l,1,max_sent_l], tf.float32) # shape : (?, 1, ?)
        ...
        tokens_output = LReLu(tf.tensordot(tf.concat([tokens_sem, tokens_output_], 2), w_comb_s, [[2], [0]]) + b_comb_s) # (18)
