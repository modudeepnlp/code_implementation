...
class Model(object):
    ...
    def build_model(self):
        with tf.variable_scope("Input_Embedding_Layer"):
            ...
            if self.use_feat:
                c_emb = tf.concat([c_emb, self.cont_feat], axis=-1) ## concat [context_pos, context_ner, context_match]
                q_emb = tf.concat([q_emb, self.ques_feat], axis=-1) ## concat [ques_pos, ques_ner, ques_match]
